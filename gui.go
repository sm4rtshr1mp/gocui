...
func (g *Gui) execKeybindings(v *View, ev *gocuiEvent) (matched bool, err error) {
	...
	for _, kb := range g.keybindings {
		...
		if kb.matchView(v) {
			return g.execKeybinding(v, kb, keyEv)
		}
	...
	if globalKb != nil {
		return g.execKeybinding(v, globalKb, globalKeyEv)
	}
...
func (g *Gui) execKeybinding(v *View, kb *keybinding, keyEv *KeyEvent) (bool, error) {
